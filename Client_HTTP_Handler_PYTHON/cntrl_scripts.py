...
"""
def move_up_measure_force(http):
    ...
    for i in range(100):
        http.request(f"MVTL:0000 0000 -00{z_delta}")

        pos_info.append((http.read_port()).strip("[]").split(","))

        real_force_info.append(np.linalg.norm((http.read_port()).strip("[]").split(",")))
...
def joint_test(http):
    ...
    for x in test_jnts:
        resp = http.request_resp(f"STJT:{x}")
        print(resp)
...
def latency_test(http):
    ...
    for i in range(100):
        ...
        resp = http.request_resp(f"ECHO:{test_str}")
